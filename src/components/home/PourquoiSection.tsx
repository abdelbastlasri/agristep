  ...
  const items = [
    {
      key: "expertise",
      icon: (
        <svg aria-hidden="true" className="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div ref={ref} className={`grid sm:grid-cols-2 md:grid-cols-4 gap-6 stagger-children ${isVisible ? 'visible' : ''}`}>
          {items.map((item) => (
            <div
          key={item.key}
          className="bg-premium-card/50 backdrop-blur-sm border border-premium-border rounded-2xl p-5 sm:p-6 text-center group hover:bg-premium-card hover:border-premium-border transition-all duration-300"
        >
          <div className="w-14 h-14 rounded-full bg-premium-green/15 flex items-center justify-center mx-auto mb-6 group-hover:scale-110 group-hover:bg-premium-green/25 transition-all duration-300">
            {item.icon}
          </div>
